...
<img src="/assets/kernel-space.png" alt="Trust boundaries: kernel space (Landlock, seccomp, Seatbelt), sandboxed child (untrusted), and supervisor (trusted)" />
...
<img src="/assets/child-flow.png" alt="Child open() flow: seccomp BPF traps openat, supervisor decides, fd injected or Landlock enforces" />
...
| `connect` / `bind` | No | Governed by Landlock ABI v4+ network filtering |
...
| Network | None — the supervisor does not listen on any port |
