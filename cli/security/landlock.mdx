...
### No Deny-Within-Allow Semantics

Landlock is allow-list only. It cannot enforce "deny a child path under an allowed parent."  
Example: allowing `/tmp` while denying `/tmp/.credentials` is not enforceable by Landlock.

nono treats this as a configuration error on Linux and refuses to start when such overlap is detected, rather than running with a silently ineffective deny rule.
