...
The `undo` section configures which paths are excluded from undo snapshots when using `--supervised` mode. This is useful for excluding build artifacts, caches, or other files that don't need restoration.
...
| Field | Description |
|-------|-------------|
| `exclude_patterns` | Directory/file names to exclude (matched by component) |
| `exclude_globs` | Glob patterns to exclude |

These exclusions apply in addition to gitignore rules (if present in the tracked directories). Excluding large directories like `node_modules` significantly improves snapshot performance.
...
## macOS Keychain Exception

On macOS, explicitly granting `login.keychain-db` in a profile (for example via `filesystem.read_file`) enables a keychain compatibility exception in the generated Seatbelt profile. nono skips default denies for `com.apple.SecurityServer` and `com.apple.securityd` Mach lookups so keychain-backed auth flows can run.

This is intentional but broader than file-only semantics, because keychain operations are mediated through Mach IPC rather than direct file reads.
