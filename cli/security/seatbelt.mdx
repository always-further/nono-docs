...
; System paths from policy.json groups
...
nono allows read access to system paths required for running executables. These are defined in `policy.json` groups (`system_read_macos`, `system_read_linux`):
...
<Warning>
  If you explicitly grant `~/Library/Keychains/login.keychain-db` (or `/Library/Keychains/login.keychain-db`), nono relaxes the default deny rules for `com.apple.SecurityServer` / `com.apple.securityd` Mach lookups so keychain-backed auth flows can work.

  This is broader than file-level access alone: keychain operations are mediated via Mach IPC, so allowing `login.keychain-db` can implicitly enable access to other keychain items available to the calling user context.
</Warning>
